...
func FuzzQueryJsonTextP(db *gorm.DB, jsonField string, search *string) *gorm.DB {
	...
	return FuzzQueryJsonText(db, jsonField, webutil.Str(search))
}
...
func Paging(db *gorm.DB, page int, limit int, data interface{}) (*pagination.Paginator, *gorm.DB) {
	p, db := pagination.Paging(&pagination.Param{
		DB:    db,
		Page:  page,
		Limit: limit,
	}, data)
...
func PagingP(db *gorm.DB, page *int64, limit *int64, data interface{}) (*pagination.Paginator, *gorm.DB) {
	var (
		pageInt, limitInt = 1, 10
	)
	if page != nil {
		pageInt = int(webutil.Int64(page))
	}
	if limit != nil {
		limitInt = int(webutil.Int64(limit))
	}
...
func FuzzSearchP(db *gorm.DB, fields []string, target *string) *gorm.DB {
	...
	return FuzzSearch(db, fields, webutil.Str(target))
}
